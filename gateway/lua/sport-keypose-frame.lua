...
for i, item in ipairs(items) do
    if item.framePosition ~= nil then
        item.poseModelImage =
            ...
            '/sport/keypose_frame?frame_position=' .. item.framePosition .. '&video_url=' .. request_body.vidoSrc
        )
        ngx.say(json.encode(item.poseModelImage))
    end
end
...
-- table.insert(
--     requests,
--     {'http://ai.f.gateway.host.smallsaas.cn:81/api/crud/coachingAction/coachingActions/' + id, {method = ngx.HTTP_PUT}}
-- )
-- table.insert(requests, {'http://localhost:5000/keypose_frame'})
